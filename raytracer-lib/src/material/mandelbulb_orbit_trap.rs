...
impl Material for MandelbulbOrbitTrap {
	fn base_colour(&self, isect : &Intersection) -> Colour {
        ...
        for _ in 0..self.iterations {
            ...
            w = isect.point + r.powi(self.power) 
				* Vec3d::new(b.sin() * a.sin(), b.cos(), b.sin() * a.cos());
		...
		col = mix( col, Vec3d::new(0.10,0.20,0.30), clamp(trap.y,0.0,1.0) );
	 	col = mix( col, Vec3d::new(0.02,0.10,0.30), clamp(trap.z*trap.z,0.0,1.0) );
        col = mix( col, Vec3d::new(0.30,0.10,0.02), clamp(trap.w.powi(6),0.0,1.0) );
        col *= 0.5;
		col = sqrt(col);
