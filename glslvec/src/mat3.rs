...
impl<T> Mat3<T>
where
    T: Sized + Copy + HasTrig + One + Mul<Output = T> + Sub<Output = T> + Add<Output = T>,
    Vec3<T>: HasNormalize,
{
...
impl<T> Mat3<T>
where
    T: Copy + Sized + Mul<Output = T> + Sub<Output = T> + Add<Output = T> + One + Div<Output = T>,
{
...
where
    T: Copy + Sized + Mul<Output = T> + Add<Output = T>,
{
    type Output = Self;
